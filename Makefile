# Description: Makefile for Python MCP Karpenter Documentation Server

PROJECT_NAME := mcp-karpenter-documentation
TARGET := ./target
...
index: ## Build the documentation index
	...
	$(PYTHON) -m mcp_karpenter_documentation.cli index

.PHONY: run
run: ## Run the MCP server
	@echo "==> Running MCP server..."
	$(PYTHON) -m mcp_karpenter_documentation.server

DOCKER_IMAGE := mcp-karpenter-documentation
DOCKER_REGISTRY := martoc
DOCKER_TAG := latest
...
docker-tag: ## Tag Docker image for registry
	...
	docker tag $(DOCKER_IMAGE) $(DOCKER_REGISTRY)/$(DOCKER_IMAGE):v$(shell grep version pyproject.toml | head -1 | sed 's/.*"\(.*\)".*/\1/')

.PHONY: docker-push
docker-push: docker-tag ## Push Docker image to registry
	@echo "==> Pushing Docker image..."
	docker push $(DOCKER_REGISTRY)/$(DOCKER_IMAGE):$(DOCKER_TAG)
	docker push $(DOCKER_REGISTRY)/$(DOCKER_IMAGE):v$(shell grep version pyproject.toml | head -1 | sed 's/.*"\(.*\)".*/\1/')
