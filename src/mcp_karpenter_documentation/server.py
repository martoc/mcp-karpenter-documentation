"""FastMCP server for Karpenter documentation search and retrieval."""
...
DEFAULT_DB_PATH = Path(__file__).parent.parent.parent / "data" / "karpenter_docs.db"

# Initialise FastMCP server
mcp = FastMCP(name="karpenter-documentation")
...
def search_documentation(
    ...
    Args:
        query: Search terms to find in the documentation. Supports
               full-text search with stemming (e.g., "scale" matches
               "scaling", "scaled").
        section: Optional section to filter results. Common sections include:
                 'docs', 'guides', 'concepts', 'reference', 'troubleshooting', etc.
...
def read_documentation(path: str) -> str:
    ...
    Args:
        path: The relative path to the documentation file (e.g.,
              'docs/concepts/scheduling.md' or 'guides/getting-started.md').
