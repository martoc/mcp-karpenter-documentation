"""Command-line interface for Karpenter documentation indexer."""
...
from mcp_karpenter_documentation.server import DEFAULT_DB_PATH
...
def cmd_index(args: argparse.Namespace) -> int:
    ...
    indexer = KarpenterDocsIndexer(database)
...
def cmd_stats(args: argparse.Namespace) -> int:
    ...
    if not db_path.exists():
        ...
        logger.info("Run 'karpenter-docs-index index' to create the index")
        return 1
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="karpenter-docs-index",
        description="Karpenter documentation indexer for MCP server",
    )
    parser.add_argument(
        "--database",
        "-d",
    ...
    # Index command
    index_parser = subparsers.add_parser("index", help="Index Karpenter documentation")
    index_parser.add_argument(
        "--branch",
        "-b",
        default="main",
        help="Git branch to index (default: main)",
    )
    index_parser.add_argument(
        "--rebuild",
        "-r",
